...
- [x] 10.5 Deploy behind the existing feature flag and record project 26 input size, model duration, total duration, storyboard coverage, material bindings, call count, and point settlement against execution 91399.

  Production evidence (2026-09-05): release `202609051124-8007ee8` is active with known-good rollback target `202609050453-2f8710c`; service/API/homepage checks returned `active`/`401`/`200`. Project 26 execution `94278`, Run `477`, reduced the five audited read results from 240,260 to 14,123 characters (94.1%). Its single model call `1379` took 37,236 ms versus 107,899 ms for baseline execution `91399` (65.5% lower); total execution time was 39 s versus 110 s (64.5% lower). The successful atomic save produced four contiguous storyboards covering `S0003` through `S0042`, each with three internal shots; all four have valid shot/prompt JSON and `BOUND` material status. The execution reserved and settled 10 points with no release. Initial post-deploy execution `94169` exposed an adjacent-output Schema mismatch before provider contact; it released all 10 reserved points and preserved the prior six storyboards. Commit `8007ee8` added the missing excerpt fields to the strict Schema with a regression test before the successful acceptance run.
