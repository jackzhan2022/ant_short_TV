...
- [x] 1.2 Add failing authorization cases covering creator-owned unbound drafts, tenant-wide viewers, active bound-project members, removed members, inaccessible projects, and cross-tenant rows for both new reads.
...
- [x] 2.1 Add failing persistence tests proving the summary read returns the same accessible project set for small and larger fixtures without increasing query count per project.
...
- [x] 2.4 Run the focused backend controller and persistence tests and confirm the summary contract, authorization parity, and constant query bound pass.

## 3. Implement Batch Metrics Without Long-Text Reads

- [x] 3.1 Add failing service/integration tests for version count, latest-task selection, every existing derived review state, outstanding-issue count, empty projects, and multiple projects in one metrics response.
...
- [x] 3.4 Add SQL-capture/query-bound assertions showing both new reads use a fixed number of statements as project count increases and do not select known long-text columns.
- [x] 3.5 Run the focused backend tests and confirm metric values, authorization, query bounds, and column boundaries pass.
...
- [x] 4.3 Regenerate the OpenAPI client with `npm run openapi` after the backend exposes the new contracts; do not edit `frontend/src/services/ant-design-pro/` manually.
...
- [x] 5.3 Add failing library tests for one successful metrics merge, metrics failure with rows preserved, metrics-only retry, import-triggered summary refresh followed by enrichment, and suppression of stale metric responses.
...
- [x] 7.1 Run backend review/schema tests and the full backend test suite, recording that the new summary and metrics reads pass access and query-bound coverage.
- [x] 7.2 Run frontend unit tests, `npm run tsc`, `npm run biome:lint`, `npx antd lint ./src`, and the production build; resolve only regressions introduced by this change.
- [x] 7.3 In a production-like MySQL environment, compare `EXPLAIN ANALYZE`, selected columns, query count, and payload size for legacy, summary, and metrics reads using representative tenants.
- [x] 7.4 Verify backend-first compatibility, summary-first rendering, metrics failure/retry, legacy endpoint rollback, and separate p50/p95 measurements for time-to-summary and time-to-metrics.
