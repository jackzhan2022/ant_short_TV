...
const toVideoInitialValues = (
  ...
  prompt: storyboard?.videoPrompt || storyboard?.visualDescription || '',
...
const ScriptCreationWorkspace = ({
  ...
  }, [canViewAiVideoTasks, message, projectId]);
  ...
  useEffect(() => {
    loadVideoTasks();
    loadVideoServices();
  }, [loadVideoServices, loadVideoTasks]);

  useEffect(() => {
    const activeTasks = videoTasks.filter((task) =>
      ['PENDING', 'SUBMITTING', 'GENERATING'].includes(task.status),
    );
      ...
      },
      {
        ...
        render: (_, record) => (
          <Space>
            ...
            {['PENDING', 'SUBMITTING', 'GENERATING'].includes(record.status) ? (
              <>
                <Button
                  type="link"
                  icon={<ReloadOutlined />}
          ...
          <Space>
            {canCreateAiVideoTasks && (
              <Button
                type="link"
                icon={<VideoCameraOutlined />}
                onClick={() => openVideoCreate(record)}
              >
                生成视频
              </Button>
            )}
