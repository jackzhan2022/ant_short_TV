...
| --- | --- | --- |
| Image generation | Provider AI | Durable shared execution, invocation, usage/cost, and point reservation/settlement |
...
| AI video generation and provider polling | Provider AI | Shared invocation/attempt/usage/cost/settlement records with provider-native asynchronous polling |
...
| AI service connectivity test | Administrative diagnostic | Credential/endpoint/model validation only. It is synchronous and does not create a user-facing execution or bill points. |

## Legacy compatibility paths

`AiTextGenerationService` has no production caller and remains only as legacy compatibility code covered by its own tests. It must not be used by new workflows. Its direct HTTP and manual call-log implementation is retired with the other legacy gateways in task 10.5.

The parameterless-context overloads in script analysis and review are legacy internal compatibility paths. Controllers and registered handlers use execution-aware overloads, so normal user traffic reserves and settles points through the shared execution.
