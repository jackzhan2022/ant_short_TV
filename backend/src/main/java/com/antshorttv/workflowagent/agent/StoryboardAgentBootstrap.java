...
public class StoryboardAgentBootstrap extends AbstractAnalysisAgentBootstrap {
    ...
    @Override protected WorkflowAgentCommand definition(Long modelId) {
        return new WorkflowAgentCommand(
            ...
            "服务端已准备完整可信上下文。严格按 Skill 用 schemaVersion 3 完成整集分镜；为每个分镜提交创意终点 sourceTo，并可为内部镜头提交非递减 sourceAnchor。不要枚举 soundSegmentIds，声音归属和可信原文由后端派生。将表演、情绪和运镜分别写入对应字段，不要混为机械校验条件。只调用 save_episode_storyboards，并以保存成功作为终止动作。",
