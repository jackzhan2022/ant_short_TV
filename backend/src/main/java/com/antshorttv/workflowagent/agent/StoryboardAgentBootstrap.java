...
public class StoryboardAgentBootstrap extends AbstractAnalysisAgentBootstrap {
    ...
    @Override protected WorkflowAgentCommand definition(Long modelId) {
        return new WorkflowAgentCommand(
            ...
            "服务端已准备完整可信上下文。严格按 Skill 用 schemaVersion 2 完成整集分镜；sourceFrom 和 sourceTo 必须放在每个分镜对象内部，根对象禁止出现。soundSegmentIds 只能引用 type 为 DIALOGUE、NARRATION 或 INNER_OS 的来源 ID，禁止引用 ACTION 或 METADATA。只调用 save_episode_storyboards，并以保存成功作为终止动作。",
